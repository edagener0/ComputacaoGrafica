...
class TextureMaterial(Material):
    def __init__(self, texture1, texture2, property_dict={}):
        vertex_shader_code = """
        ...
        uniform sampler2D texture1; // topo
        uniform sampler2D texture2; // lados
        ...
        void main()
        {
            vec4 texTop = texture(texture1, UV);
            vec4 texSide = texture(texture2, UV);

            float factor = abs(normal.y);

            vec4 color = mix(texSide, texTop, factor);
