...
class Example(Base):
    ...
    def initialize(self):
        ...
        self.renderer = Renderer(clear_color=[0.0, 0.0, 0.0])
        self.scene = Scene()
        ...
        # three light sources
        ambient_light = AmbientLight(color=[0.0, 0.0, 0.0])
        self.scene.add(ambient_light)
        self.directional_light = DirectionalLight(color=[0.8, 0.8, 0.8], direction=[-1, -1, 0])
        self.directional_light2 = DirectionalLight(color=[0.0, 0.0, 1.0], direction=[-1, 1, 0])
        ...
        lambert_material = PhongMaterial(
            property_dict={"baseColor": [0.2, 0.5, 0.5]},
            number_of_light_sources=4,
            use_shadow=True
        )
        ...
        self.directional_light2.set_position([0, -3, 0])
